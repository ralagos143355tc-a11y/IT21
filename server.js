...
function loadTemplate(name) {
  ...
  console.error(`Failed to load public/${name} from`, candidates);
  return null;
}
...
<body style="font-family:system-ui;max-width:40rem;margin:3rem auto;padding:0 1rem;line-height:1.5">
  ...
  <p>Then open <strong>Deployments → Redeploy</strong> (env vars apply only after redeploy).</p>
</body></html>`);
    return;
  }
  next();
...
if (configOk) {
  ...
  app.get('/login', redirectIfAuthenticated, ensureCsrfToken, (req, res) => {
    if (!loginTemplate) {
      res.status(500).send('login.html failed to load.');
      return;
    }
  ...
  app.get('/home', requireAuth, (req, res) => {
    if (!homeTemplate) {
      res.status(500).send('home.html failed to load.');
      return;
    }
